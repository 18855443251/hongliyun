...
<template>
  <div>
    ...
    <div>7、 执⾏同步任务 console.log(11)， 输出 11 ；</div>
    <div>8、 从微任务队列取出任务 then1 到主线程中，输出 9 ；</div>
    <div>
      9、 从微任务队列取出任务 then2 到主线程中，输出 10 ，⾄此微任务队列为空；
    </div>
    <div>
      10、从宏任务队列中取出 time2( 注意这⾥不是 time1 的原因是 time2
      的执⾏时间为 0)；
    </div>
    <div>11、 执⾏同步任务 console.log(5) ，输出 5 ；</div>
    <div>
      12、new Promise 在实例化过程中所执⾏的代码都是同步执⾏的（ function
      中的代码），输出6 ；
    </div>
    <div>
    ...
    <div>
      17、new Promise 在实例化过程中所执⾏的代码都是同步执⾏的（ function
      中的代码），输出3 ；
    </div>
    <div>
